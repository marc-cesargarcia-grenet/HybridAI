...
def train_mlp(a, b):
    ...
    for epoch in range(EPOCHS):
        ...
        # Train on interior points (MSE loss)
        for x_batch, y_batch, f_batch, u_fd_batch in interior_loader:
            ...
            # MSE loss against finite difference solution
        ...
        if (epoch + 1) % 100 == 0:
            ...
            print(f"Epoch {epoch+1}/{EPOCHS}, Loss: {epoch_loss:.6f}, MSE Loss: {avg_mse_loss:.6f}, BC Loss: {avg_bc_loss:.6f}")
    ...
    torch.save(model.state_dict(), SAVE_PATH_DIR / f"mlp_a{a}_b{b}.pth")
    ...
    plt.title(f'MLP Training Losses (a={a}, b={b})')
    plt.legend()
    plt.grid(True)
    plt.savefig(f"fig/mlp_loss_a{a}_b{b}.png")
    
    return model

# ------------------------------
# Evaluate and visualize solution
def evaluate_mlp(model, a, b, compare_with_fd=False):
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    ...
    model.eval()
    with torch.no_grad():
        u_mlp = model(x_flat, y_flat).reshape(n_viz, n_viz).cpu().numpy()
    ...
    # Plot MLP solution
    im1 = axes[1].imshow(u_mlp, origin='lower', extent=[0, 1, 0, 1], cmap='viridis')
    axes[1].set_title('MLP Solution u(x,y)')
    axes[1].set_xlabel('x')
    axes[1].set_ylabel('y')
    plt.colorbar(im1, ax=axes[1])
    
    plt.tight_layout()
    plt.savefig(f"fig/mlp_solution_a{a}_b{b}.png")
    ...
    if compare_with_fd:
        ...
        try:
            ...
            if idx is not None:
                ...
                # Compute error
                error = np.abs(u_mlp - u_fd_resized)
                
                # Visualize comparison
                fig, axes = plt.subplots(1, 3, figsize=(18, 5))
                
                im0 = axes[0].imshow(u_mlp, origin='lower', extent=[0, 1, 0, 1], cmap='viridis')
                axes[0].set_title('MLP Solution')
                axes[0].set_xlabel('x')
                axes[0].set_ylabel('y')
                plt.colorbar(im0, ax=axes[0])
                ...
                plt.tight_layout()
                plt.savefig(f"fig/mlp_vs_fd_a{a}_b{b}.png")
                
                # Compute error metrics
                l2_error = np.sqrt(np.mean((u_mlp - u_fd_resized)**2))
                max_error = np.max(np.abs(u_mlp - u_fd_resized))
                
                print(f"L2 Error: {l2_error:.6e}")
                print(f"Max Error: {max_error:.6e}")
                
                return u_mlp, u_fd_resized, l2_error, max_error
            else:
